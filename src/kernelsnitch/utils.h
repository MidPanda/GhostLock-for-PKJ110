...
/* Runtime verbose gate: pr_info (diagnostic detail) prints only when the
 * env var DEBUG=1 is set. Cached per TU after first call.
 * pr_success / pr_error / pr_warning always print (state + verdicts). */
static inline int gl_debug(void) {
    static int cached = -1;
    if (cached < 0) {
...
#define pr_info(fmt, ...) do { \
        if (gl_debug()) { __android_log_print(ANDROID_LOG_INFO, "google_poc_app", "[*] %s:%d " fmt, __FILE__, __LINE__, ##__VA_ARGS__); } \
    } while (0)
...
#define pr_info(fmt, ...) do { \
        if (gl_debug()) { printf(COLOR_YELLOW "[*] %s:%d " COLOR_DEFAULT fmt, __FILE__, __LINE__, ##__VA_ARGS__); } \
    } while (0)
#define pr_success(fmt, ...) do { \
...
#define pr_info(fmt, ...) do { \
        if (gl_debug()) { __android_log_print(ANDROID_LOG_INFO, "google_poc_app", "[*] " fmt, ##__VA_ARGS__); } \
    } while (0)
#define pr_success(fmt, ...) do { \
...
#define pr_info(fmt, ...) do { \
        if (gl_debug()) { printf(COLOR_YELLOW "[*] " COLOR_DEFAULT fmt, ##__VA_ARGS__); } \
    } while (0)
#define pr_success(fmt, ...) do { \
